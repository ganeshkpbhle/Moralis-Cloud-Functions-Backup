const contractIdlist=[
  ...
  Moralis.Cloud.define('getStatus',async(request)=>{
			let ChainColl="Transactions";
			const trans=await new Parse.Query(ChainColl).equalTo('Network',request.params.chain.toString()).aggregate([
				  {
					  ...
					  pipeline:[
						{
						  $match:{
							  $and:[
									{'senderAddress':{$eq:[request.params.addr]}},{'Network':{$eq:[request.params.chain.toString()]}}
								]
							//'senderAddress':request.params.addr
						  }
						}
					  ]
					}
